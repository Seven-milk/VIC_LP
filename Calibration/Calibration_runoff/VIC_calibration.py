...
class spotpy_setup():
    ...
    def __init__(self, stations=None, start_run="19830101", end_run="19981231", start_cali="19840101",
                 end_cali="19981231", skip_year="1"):
        """
        init function
            input:
        ...
        self.params = [spotpy.parameter.Uniform('binfilt', 0, 0.4),
                       spotpy.parameter.Uniform('Ds', 0., 1),
                       spotpy.parameter.Uniform('Dsmax', 0, 30),
                       spotpy.parameter.Uniform('Ws', 0.0, 1.),
                       spotpy.parameter.Uniform('depth1', 0.1, 0.5),
                       spotpy.parameter.Uniform('depth2', 0.3, 1.5),
                       spotpy.parameter.Uniform('depth3', 0.3, 2.0),
                       ]
        return

    def simulation(self, params):
        """ simulation based on params """
        self.vicmodel.run_vic(binfilt=params[0], Ds=params[1], Dsmax=params[2], Ws=params[3], depth1=params[4],
                              depth2=params[5], depth3=params[6],
...
def findBestSim(dbPath):
    ...
    Dsmax = results[idx, 3]
    Ws = results[idx, 4]
    depth1 = results[idx, 5]
    depth2 = results[idx, 6]
    depth3 = results[idx, 7]

    params = [binfilt, Ds, Dsmax, Ws, depth1, depth2, depth3]
...
def calibrate(parallel_spotpy="seq", plot=False):
    ...
    # plot
    if plot:
        plot_calibration(out_csv=outCal + ".csv", start_cali=start_cali, end_cali=end_cali, stations=stations)

    return


def plot_calibration(out_csv, start_cali="19840101", end_cali="19891231", stations=None):
    ...
    lastRun.run_vic(params[0], params[1], params[2], params[3], start=start_cali, end=end_cali, skip_year="0")
