...
def Evaluate(simu, obs):
    ...
    ax.plot(simu, 'r.', markersize=3, label='Simulation')
    ax.plot(obs, color='black', linestyle='solid', label='Observation data')
    plt.xlabel('Number of Observation Points')
    plt.ylabel('Data')
    plt.legend(loc='upper right')
    fig.savefig('SCEUA_best_modelrun_Mannual.png', dpi=300)

    # cal evaIndicators
    ei = EvaIndicators(simu, obs)
...
def main():
    ...
    skip_year = "1"
    vic = vic_model()
    params = dict(binfilt=0.349927, Ds=0.351823, Dsmax=10.0359, Ws=0.782321, depth3=1.53088)
    ...
    vic_obs = vic.get_obs(start_cali, end_cali)
    # np.save(f"Cali_SoilMoi0_100cm_inst_{start_cali}_{end_cali}_D_vic", vic_cali)

    # evaluation
    evaluation = Evaluate(vic_cali, vic_obs)

    with open("Calibration_GLDAS_manual_result", "a") as f:
        params_f = f'binfilt={params["binfilt"]}, Ds={params["Ds"]}, Dsmax={params["Dsmax"]}, Ws={params["Ws"]}, depth3={params["depth3"]}\n'
        evaluation_f = f'RMSE={evaluation["RMSE"]}, RRMSE={evaluation["RRMSE"]}, R={evaluation["R"]}, Ce={evaluation["Ce"]}, Bias={evaluation["Bias"]}\n'
        f.write(params_f)
        f.write(evaluation_f)
        f.write("----------------------\n")
    ...
    # np.save(f"Vali_SoilMoi0_100cm_inst_{start_vali}_{end_vali}_D_vic", vic_simu)
